...
Pravidlový engine počítá EMA 20/50/200, RSI 14, ATR 14 a price action posledních svíček. LONG nebo SHORT vrátí pouze při silné shodě všech tří timeframe; jinak zůstává NO TRADE. U NO TRADE doporučí další vhodný čas analýzy podle timeframe, který vstup blokuje. Server stáhne minutová data pouze jednou a lokálně z nich vytvoří M5 i M15; H1 načte samostatně z hodinového endpointu. Každá datová větev stahuje soubory postupně s identifikací aplikace a při dočasném omezení zdroje požadavek opakuje s rostoucím odstupem. Současně se slučují souběžné analýzy stejného instrumentu a hotový tržní kontext se krátce cachuje, aby zbytečně nevznikaly duplicitní požadavky. Před výpočtem signálu server navíc kontroluje poslední uzavřenou svíčku: H1 smí být stará nejvýše 120 minut, M15 nejvýše 30 minut a M5 nejvýše 15 minut. Při překročení kteréhokoli limitu analýzu zablokuje jako `NO TRADE – neaktuální data` a zobrazí čas i stáří každého timeframe; taková analýza se neuloží do databáze. Referenční cenu a čas načítá automaticky ze stejného minutového datasetu Dukascopy a bezpečně přechází na čerstvou M5 nebo H1, pokud M1 není dostupná. Zastaralou referenci odmítne. Aktivní signál obsahuje také odhad doporučené doby držení a časový stop odvozený z ATR na M15.
